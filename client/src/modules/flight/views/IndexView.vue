...
const flights = ref([]), loadingFlights = ref(false);
const page = ref(0), recordPerPage = ref(10), orderBys = ref([{column: 'created_at', type: 'desc'}]);
const isConfirmDeleteModalShown = ref(false);
const flightToBeDeleted = ref(null);

function fetchFlights() {
    loadingFlights.value = true;

    usePTechFlight().paginate(unref(page), unref(recordPerPage), unref(orderBys)).then(listData => {
        flights.value = listData;
    }).finally(() => {
...
function deleteFlight(flight) {
    console.log(flightToBeDeleted.value = flight);
    isConfirmDeleteModalShown.value = true;
}

function confirmToDeleteFlight() {
    console.log(flightToBeDeleted)
    usePTechFlight().remove(flightToBeDeleted.value.uuid).then(() => {
        router.go();
    });
}
        ...
        <div class="py-2 px-4">
            <div class="relative overflow-x-auto">
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                    <tr v-for="flight in flights" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <td class="w-4 p-4">
                            <div class="flex items-center">
                        ...
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <router-link :to="{name: 'flight.edit', params: {'uuid': flight.uuid}}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">{{flight.name}}</router-link>
                        </th>
                        <td class="px-6 py-4">
                            {{truncate(flight.description, 50)}}
                        </td>
                        <td class="px-6 py-4">
                            {{datetime(flight.created_at)}}
                        </td>
                        <td class="px-6 py-4">
                            <a href="#" class="font-medium text-blue-600 dark:text-blue-500 hover:underline" @click.prevent="deleteFlight(flight)">Delete</a>
                        </td>
                    </tr>
                    </tbody>
                </table>
